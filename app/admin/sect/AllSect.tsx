...
import ModalEditSect from "./ModalEditSect";
import ModalDeleteSect from "./ModalDeleteSect";
import ModalCreateSect from "./ModalCreateSect";
      ...
      <div className="overflow-x-auto">
        <Table hoverable>
          <Table.Head className="text-md">
            <Table.HeadCell>ชื่อแขนง</Table.HeadCell>
            ...
            {data &&
              data?.sects?.map((item: any) => (
                <Table.Row key={item?._id}>
                  ...
                  <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">
                    {item?.major?.program?.name}
                  </Table.Cell>
                  {/* <Table.Cell>{item.email}</Table.Cell> */}
                  <Table.Cell className="flex gap-2">
                    <ModalEditSect data={item} refetch={refetch} />
                    <ModalDeleteSect data={item} refetch={refetch} />
                  </Table.Cell>
                </Table.Row>
              ))}
          </Table.Body>
